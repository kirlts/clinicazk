...
| `src/assets/`, `src/icons/` | **[Recursos estáticos]** Logos de marca (`brand/`), fotos de sedes (`fotos/`), retratos del equipo (`equipo/`), fotos por especialidad (`especialidades/`, nombradas por su posición 1-14 en `ESPEC`) e íconos. El glob es eager: una imagen sin uso igual se copia al build. | Al actualizar recursos de marca o imágenes. |
